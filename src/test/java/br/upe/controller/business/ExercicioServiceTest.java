...
class ExercicioServiceTest {
    ...
    void testAtualizarExercicio_NomeAtualVazio() {
        ...
        assertEquals("Nome atual inválido.", exception.getMessage());
    }
    ...
    void testAtualizarExercicio_NaoEncontrado() {
        ...
        assertEquals("Exercício 'Exercicio 1' não encontrado.", exception.getMessage());
    }

    @Test
    @DisplayName("Deve lançar exceção para novo nome já existente")
    void testAtualizarExercicio_NovoNomeJaExiste() {
        Exercicio outroExercicio = new Exercicio();
        outroExercicio.setId(2);
        outroExercicio.setUsuario(usuario);
        outroExercicio.setNome("Outro");
        outroExercicio.setDescricao("Desc");
        outroExercicio.setCaminhoGif("gif");
        ...
        assertEquals("Já existe um exercício com o nome 'Outro'.", exception.getMessage());
    }

    @Test
    @DisplayName("Deve lançar exceção quando usuário não encontrado no cadastro")
    void testCadastrarExercicio_UsuarioNaoEncontrado() {
        when(exercicioRepository.buscarTodosDoUsuario(1)).thenReturn(List.of());
        when(usuarioRepository.buscarPorId(1)).thenReturn(Optional.empty());

        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () -> {
            exercicioService.cadastrarExercicio(1, "Exercicio Novo", "Descricao", "caminho.gif");
        });
