...
class IndicadorBiomedicoServiceTest {
    ...
    void testGerarRelatorioPorData_Success() {
        ...
        List<IndicadorBiomedico> indicadores = new ArrayList<>(List.of(indicador));
    ...
    void testGerarRelatorioPorData_Ordenado() {
        ...
        IndicadorBiomedico ind1 = new IndicadorBiomedico();
        ind1.setDataRegistro(LocalDate.of(2023, 6, 1));
        ind1.setPesoKg(70.0);

        IndicadorBiomedico ind2 = new IndicadorBiomedico();
        ind2.setDataRegistro(LocalDate.of(2023, 1, 15));
        ind2.setPesoKg(72.0);

        when(indicadorRepository.buscarPorPeriodo(1, inicio, fim)).thenReturn(new ArrayList<>(List.of(ind1, ind2)));

        List<IndicadorBiomedico> result = indicadorService.gerarRelatorioPorData(1, inicio, fim);

        assertEquals(2, result.size());
        assertTrue(result.get(0).getDataRegistro().isBefore(result.get(1).getDataRegistro()));
    }
